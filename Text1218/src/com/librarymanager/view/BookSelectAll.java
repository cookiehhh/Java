...
public class BookSelectAll extends Panel implements ActionListener{
	/**
	 * 
	 */
	...
	public BookSelectAll(Window parent) {
 	 
      sel = new JButton("查询");
      sel.addActionListener(this);
      jtf = new JTextField(25);
      jtf.setFont(new Font("宋体",Font.BOLD,25));
      la1 = new JLabel("查询全部图书");
      ...
      String[] colnames = { "图书编号", "图书名称", "图书类型", "价格", "作者","出版社","库存"};
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("查询")) {						
			try {
				ps = (PreparedStatement)conn.prepareStatement("select * from book");
				rs = ps.executeQuery();
				int i=0;
				while (rs.next()) {
					...
					table.setValueAt(rs.getString(7).trim(), i, 6);
					i++;
