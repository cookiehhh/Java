...
public class TypeSelectAll extends Panel implements ActionListener{
	/**
	 * 
	 */
	...
	public TypeSelectAll(Window parent) {
 	 
      sel = new JButton("查询");
      sel.addActionListener(this);
      jtf = new JTextField(25);
      jtf.setFont(new Font("宋体",Font.BOLD,25));
      la1 = new JLabel("查询全部类型");
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("查询")) {						
			try {
				ps = (PreparedStatement)conn.prepareStatement("select * from type");
				rs = ps.executeQuery();
				int i=0;
				while (rs.next()) {
