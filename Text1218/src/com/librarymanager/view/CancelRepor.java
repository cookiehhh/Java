...
public class CancelRepor extends Panel implements ActionListener{
	/**
	 * 
	 */
    ...
    JButton cal;
    JLabel lab;
	public CancelRepor(Window parent) {
		this.setLayout(null);

        lab = new JLabel("取消用户挂失");
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("确定")) {
			if(!usernamef.getText().isEmpty() && !passwordf.getText().isEmpty()){
				Manager.reporquery(usernamef.getText());
				if(Manager.user.getPassword() == null){
					...
					if(Manager.user.getPassword().equals(passwordf.getText())){
						...
						Manager.cancelrepor(user);
						JOptionPane.showMessageDialog(this,"取消挂失成功！","提示消息",JOptionPane.WARNING_MESSAGE);
					}
